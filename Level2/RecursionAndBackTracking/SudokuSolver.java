...
public class SudokuSolver {
  ...
  /**
 * Bonus Question 
 * To check if given values are  valid in sudoku or not 
 */
  ...
  class Solution {
    ...
    public boolean isValidSudoku(char[][] board) {
        ...
        return solveSudokuH(boardInt,0,0);
   ...
   *  in a 3 * 3 : digits 1 - 9
  **/
    public boolean solveSudokuH(int[][] board, int i, int j) {
    ...
    boolean isPa = true;
    // if not filled then skip 
    if(board[i][j] != 0) {
        int pa = board[i][j];
        board[i][j] = 0;
        isPa = isValidOption(board,i,j,pa);
        board[i][j] = pa;
    }
      
      return isPa && solveSudokuH(board,ni,nj);
  }

    public boolean isValidOption(int[][] board, int x, int y, int val) {
